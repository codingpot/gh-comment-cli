...
const GHC_CLI_INPUT_STYLE =
  'position: absolute; top: 30%; left: 50%; width:40%; font-size: 25px; color: black; transform: translate(-50%,-50%); -ms-transform: translate(-50%,-50%);'

var ghc_div_open = false
var list_of_completions = [{ label: 'dummy' }]

document.addEventListener('DOMContentLoaded', () => {
  const elem = getNewCommentElement()
  if (!elem) {
    console.warn('unable to find a comment element')
    return
  }
  ...
  autocomplete({
    ...
    minLength: 0,
    onSelect(item, input) {},
    fetch(text, update, trigger, cursorPos) {
      console.log({
        text,
        trigger,
        cursorPos,
      ...
      const nextItems = getItems(text, trigger, cursorPos)
      update(nextItems)
    },
  })
})
...
//   let gch_cli_input = <HTMLInputElement>(
//     document.getElementById(GHC_CLI_INPUT_TAG)
//   )

//   if (ghc_div_open && event.key == '!' && ghc == document.activeElement) {
//     ghc!.value = ''
...
//           completion.label.toLowerCase().startsWith(item_text),
//         )
...
//     ghc.blur()
//     gch_cli_input.focus()
//   } else if (ghc_div_open && gch_cli_input == document.activeElement) {
//     if (event.key == 'Escape') {
//       gch_cli_input.value = ''
...
//     if (element && event.key == '!' && element == document.activeElement) {
...
function getNewCommentElement(): HTMLTextAreaElement | HTMLInputElement | null {
  return document.getElementById(GHC_TAG) as
    | HTMLTextAreaElement
    | HTMLInputElement
}
